...
func (d *Driver) getIPByMACFromAPI(mac string) (string, error) {
	...
	if err != nil {
		log.Warnf("Failed to lookup network %s", d.PrivateNetwork)
		return "", err
	}
	for _, lease := range leases {
		if strings.ToLower(mac) == strings.ToLower(lease.Mac) {
...
func (d *Driver) GetIP() (string, error) {
	...
	return "", fmt.Errorf("unable to locate IP address for MAC %s", mac)
}
